...
app = FastAPI(title="mailpush API", version=VERSION)
...
def _api_token() -> str:
    with open(API_TOKEN_FILE, encoding="utf-8") as f:
        token = f.read().strip()
    if not token:
        raise RuntimeError(f"{API_TOKEN_FILE} 为空,请生成:openssl rand -hex 32")
    return token
...
@app.post("/register-token", dependencies=[Depends(require_auth)])
async def register_token(req: TokenRegister) -> dict:
    """安卓端自动上报本机 FCM token(多设备)。写入 /data(/config 是只读挂载)。"""
    token = req.token.strip()
    if not token:
        raise HTTPException(status_code=400, detail="empty token")
    added = devicetokens.add(token)
